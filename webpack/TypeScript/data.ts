...
export class dataclass{
    ...
    download_table_as_csv(table_id: string, separator: string = ','): void {
        try {
    ...
    searchEmployees(query: string): void {
        let localdata: any = JSON.parse(localStorage.getItem("data") || "{}");
        let employees: any[] = localdata.Employees;
        var results: any[] = employees.filter((employee: any) => {
        ...
        results.forEach((employee: any) => {
            let employeeDiv: HTMLDivElement = document.createElement('div');
            ...
            let name: Text = document.createTextNode(employee.firstname + " " + employee.lastname);
