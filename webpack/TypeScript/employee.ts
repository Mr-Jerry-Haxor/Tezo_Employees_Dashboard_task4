...
function Filters(): void {
    (document.querySelectorAll('.table-filters-list button') as NodeListOf<HTMLButtonElement>).forEach((button: HTMLButtonElement) => {
        button.addEventListener('click', function (event: Event) {
            ...
            const letter: string = this.textContent!;
            const index: number = selectedLetters.indexOf(letter);
            if (index !== -1) {
...
function filterTableByFirstLettersReset(): void {
    selectedLetters = [];
    filterTableByFirstLetters(selectedLetters);
...
function filterTableByFirstLetters(letters: string[]): void {
...
function checkboxes(): void {
    document.getElementById('table-header-checkbox')?.addEventListener('change', function () {
        ...
        checkboxes.forEach((checkbox: HTMLInputElement) => {
            checkbox.checked = (this as HTMLInputElement).checked;
        });
...
function checkboxIsChecked(): void {
    const checkboxes: NodeListOf<HTMLInputElement> = document.querySelectorAll('#employees-table tbody tr input[type="checkbox"]');
    checkboxes.forEach((checkbox: HTMLInputElement) => {
...
function filterEmployeesTable(): void {
...
function populateFilterOptions(selector: string, filterId: string): void {
...
function checkFilterStatus(): void {
    const statusSelect: HTMLSelectElement = document.getElementById('filter-Status') as HTMLSelectElement;
    const locationSelect: HTMLSelectElement = document.getElementById('filter-Location') as HTMLSelectElement;
    const departmentSelect: HTMLSelectElement = document.getElementById('filter-Department') as HTMLSelectElement;
...
function deleteEmployee(empid: string): void {
    try {
...
function viewmore(empid: string): void {
...
function EditEmployeeDetails(empid: string): void {
    const data: any = JSON.parse(localStorage.getItem("data") || "{}");
    if (!data || !data.Employees) {
        CustomAlert("error", "No employee data found in local storage.");
        return;
